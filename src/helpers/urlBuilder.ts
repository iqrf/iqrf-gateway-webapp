...
export default class UrlBuilder {
	 ...
	 */
	getBaseUrl(): string {
		return window.location.protocol + '//' + this.hostname + (this.isDev ? ':8081' : this.port) + import.meta.env.BASE_URL;
	}
	...
	getRestApiUrl(): string {
		return '//' + this.hostname + (this.isDev ? ':8080' : this.port) + import.meta.env.BASE_URL + '/api/v0/';
	}
	...
	getRestApiUrlFromAddr(hostname: string): string {
		return '//' + hostname + (this.isDev ? ':8080' : this.port) + import.meta.env.BASE_URL + 'api/v0/';
	}
