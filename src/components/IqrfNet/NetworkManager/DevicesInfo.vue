...
<template>
	<div>
		...
		<RestartErrorDialog ref='restartDialog' />
	</div>
</template>
...
import RestartErrorDialog from './RestartErrorDialog.vue';
	...
	private handleRestart(response): void {
		if (response.status === 0) {
			if (response.rsp.inaccessibleNodesNr > 0) {
				...
				(this.$refs.restartDialog as RestartErrorDialog).showDialog(nodes);
			} else {
				this.$toast.success(
