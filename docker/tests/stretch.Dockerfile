...
RUN apt-get update \
 ...
 && apt-get update \
 && apt-get install --no-install-recommends -y build-essential composer debhelper devscripts fakeroot git-buildpackage pkg-php-tools \
    ...
    openssh-client python3-recommonmark python3-sphinx rsync \
 && apt-get clean \
