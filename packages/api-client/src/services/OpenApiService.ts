...
export class OpenApiService extends BaseService {
	 ...
	 * @return {Promise<object>} OpenAPI specification
	 */
	public fetchSpecification(baseUrl = ''): Promise<object> {
		return this.axiosInstance.get('/openapi')
			.then((response: AxiosResponse): object => {
				...
				const spec = JSON.parse(JSON.stringify(response.data).replaceAll(regExp, replacement));
