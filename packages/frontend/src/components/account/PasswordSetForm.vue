...
<template>
	...
	>
		<ICard>
			<template #title>
				{{ $t('pages.account.invitation.title') }}
			</template>
			<p>
				{{ $t('components.account.invitation.prompt') }}
			</p>
			<IPasswordInput
			...
			<IActionBtn
				...
				:icon='mdiAccountKey'
				:text='$t("components.account.invitation.button")'
				type='submit'
			/>
		</ICard>
	</v-form>
...
async function onSubmit(): Promise<void> {
	...
	try {
		const user: UserSignedIn = await service.setPassword(componentProps.uuid, data.value);
		componentState.value = ComponentState.Success;
		store.processSignInResponse(user, true);
		await store.refreshUserPreferences();
		await router.push('/');
		toast.success(
			i18n.t('components.account.invitation.messages.success'),
		);
	} catch (error) {
		if (!(error instanceof AxiosError)) {
			componentState.value = ComponentState.Error;
			toast.error(
				i18n.t('components.account.invitation.messages.failure'),
			);
			return;
		}
		switch (error?.response?.status) {
			case 403:
				...
				toast.error(
					i18n.t('components.account.invitation.messages.blocked'),
				);
				break;
			case 404:
				componentState.value = ComponentState.NotFound;
				toast.error(
					i18n.t('components.account.invitation.messages.notFound'),
				);
				break;
			case 410:
				componentState.value = ComponentState.Expired;
				toast.error(
					i18n.t('components.account.invitation.messages.expired'),
				);
				break;
			default:
				componentState.value = ComponentState.Error;
