<template>
	...
	>
		<Card>
			<template #title>
				{{ $t('components.configuration.daemon.jsonApi.splitter.title') }}
			</template>
				...
				<v-responsive>
					<section v-if='config'>
						<TextInput
							...
							:label='$t("components.configuration.daemon.jsonApi.splitter.instanceId")'
							:rules='[
								(v: string|null) => ValidationRules.required(v, $t("components.configuration.daemon.jsonApi.splitter.validation.instanceIdMissing")),
							]'
							required
						/>
						<v-checkbox
							v-model='config.validateJsonResponse'
							:label='$t("components.configuration.daemon.jsonApi.splitter.validateResponses")'
							density='compact'
							hide-details
						/>
					</section>
...
async function onSubmit(): Promise<void> {
	...
	service.updateInstance(IqrfGatewayDaemonComponentName.IqrfJsonSplitter, instance, params)
		.then(() => {
			getConfig().then(() => {
				toast.success(
					i18n.t('components.configuration.daemon.jsonApi.splitter.messages.save.success'),
				);
			});
		})
