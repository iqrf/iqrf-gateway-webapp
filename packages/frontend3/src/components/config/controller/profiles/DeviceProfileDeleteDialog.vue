<template>
	...
	>
		...
		<Card>
			<template #title>
				{{ $t('components.configuration.controller.profiles.delete.title') }}
			</template>
			{{ $t('components.configuration.controller.profiles.delete.prompt', {name: profile.name}) }}
			<template #actions>
				<v-btn
					color='primary'
					variant='elevated'
...
async function onSubmit(): Promise<void> {
	...
	service.deleteMapping(componentProps.profile.id)
		.then(() => {
			toast.success(
				i18n.t('components.configuration.controller.profiles.messages.delete.success', {name: componentProps.profile.name}),
			);
			close();
			emit('deleted');
		})
