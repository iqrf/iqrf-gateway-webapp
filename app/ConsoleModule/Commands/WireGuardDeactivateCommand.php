 ...
 */
#[AsCommand(
	name: 'wireguard:deactivate',
	description: 'Deactivates a WireGuard tunnel',
	aliases: ['wg:deactivate'],
)]
class WireGuardDeactivateCommand extends WireGuardCommand {
	...
	protected function configure(): void {
		$this->addArgument(
			...
			'Identifier of WireGuard tunnel to deactivate',
		);
	}
	...
	protected function execute(InputInterface $input, OutputInterface $output): int {
		...
		if (!$this->manager->deleteTunnel($tunnel)) {
			$style->error('An error has occurred while deactivating WireGuard tunnel ' . $tunnelIdentifier);
			return Command::FAILURE;
		}
