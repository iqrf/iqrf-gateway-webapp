...
class FileManager implements IFileManager {
	...
	public function listDirectories(?string $subdirectory = null): array {
		...
		$files = Finder::findDirectories('*')->from($realPath)->collect();
		$files = array_map(static fn (FileInfo $file): string => Strings::substring($file->getRealPath(), $realPathLen), $files);
		sort($files);
		return $files;
	}
	...
	public function listFiles(?string $subdirectory = null): array {
		$realPath = $this->getRealPath($subdirectory);
		$realPathLen = strlen($realPath) + 1;
		$files = Finder::findFiles('*')->from($realPath)->collect();
		$files = array_map(static fn (FileInfo $file): string => Strings::substring($file->getRealPath(), $realPathLen), $files);
		sort($files);
		return $files;
	}
