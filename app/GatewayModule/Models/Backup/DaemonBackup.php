...
class DaemonBackup implements IBackupManager {
	...
	public function __construct(
		...
		$this->fileManager = new PrivilegedFileManager($daemonDirectories->getConfigurationDir(), $this->commandExecutor);
	}
	...
	public function restore(ZipArchiveManager $zipManager): void {
		...
		$this->commandExecutor->run('chown -R ' . $owner . ' ' . $this->daemonDirectories->getCacheDir(), true);
		...
		$this->commandExecutor->run('cp -rfp ' . $this->daemonDirectories->getDataDir() . 'DB/daemon/DB/* ' . $this->daemonDirectories->getDataDir() . 'DB', true);
		$this->commandExecutor->run('rm -rf ' . $this->daemonDirectories->getDataDir() . 'DB/daemon', true);
		$this->commandExecutor->run('cp -rfp ' . $this->daemonDirectories->getCacheDir() . 'daemon/scheduler/* ' . $this->daemonDirectories->getCacheDir() . 'scheduler', true);
		$this->commandExecutor->run('rm -rf ' . $this->daemonDirectories->getCacheDir() . 'daemon', true);
		$this->commandExecutor->run('cp -rfp ' . $this->daemonDirectories->getConfigurationDir() . 'daemon/* ' . $this->daemonDirectories->getConfigurationDir(), true);
		$this->commandExecutor->run('rm -rf ' . $this->daemonDirectories->getConfigurationDir() . 'daemon', true);
		$this->fixPrivileges();
	}
	...
	public function recreateDirectories(array $dirs): void {
		$owner = PosixHelper::getChownOwner();
		foreach ($dirs as $dir) {
			$this->commandExecutor->run('rm -rf ' . $dir, true);
			$this->commandExecutor->run('mkdir ' . $dir, true);
			$this->commandExecutor->run('chown ' . $owner . ' ' . $dir, true);
			$this->commandExecutor->run('chown -R ' . $owner . ' ' . $dir, true);
		}
	}
	...
	private function fixPrivileges(): void {
		$this->commandExecutor->run('chmod -R 0666 ' . $this->daemonDirectories->getConfigurationDir(), true);
		foreach ($this->fileManager->listDirectories() as $dir) {
			$this->commandExecutor->run('chmod 0777 ' . $dir, true);
		}
		$this->commandExecutor->run('chmod 0600 ' . $this->daemonDirectories->getConfigurationDir() . 'certs/core/*', true);
	}
